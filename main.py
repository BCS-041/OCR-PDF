...
# This version supports prescriptions of all types e.g. jpg/png/pdf
...
def encode_file_to_base64(file_identifier: str) -> list:
    """Convert an image or PDF (local or URL) into base64-encoded list."""
    try:
        if str(file_identifier).startswith("http"):
            ...
            if ext == "pdf":
                ...
                return [
                    base64.b64encode(BytesIO(page.save(BytesIO(), format="JPEG")).getvalue()).decode("utf-8")
                    for page in pages
                ]
...
def process_prescriptions(api_key: str):
    """Main processing pipeline for all prescriptions (URLs + local files)."""
    results = []
    ...
    if os.path.exists(INPUT_FILE):
        ...
        for idx, entry in enumerate(prescriptions, 1):
            ...
            ocr_text = extract_text_from_pdf_file(url) if ext == "pdf" else extract_text_from_image_file(url)
    ...
    if os.path.exists(IMAGES_FOLDER):
        ...
        for idx, filename in enumerate(files, len(results) + 1):
            ...
            ocr_text = extract_text_from_pdf_file(file_path) if ext == "pdf" else extract_text_from_image_file(file_path)
